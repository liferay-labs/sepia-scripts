...
RX_IGNORABLE = re.compile('^(?:\\..*|src|lib|build|classes|bundles*|liferay*)')
...
def walk(dn, keep=const(True), recur=const(True)):
    for fn in os.listdir(dn):
        ...
        # Do not turn parent projects (those which do not contain a src subfolder) into modules
        if keep(fn) and not fn.startswith('com-liferay-'):
            yield fname
...
MODULE_WITH_GROUP_XML_TEMPLATE = (
    '      <module fileurl="file://{0}" filepath="{0}" group="{1}" />'
)

MODULE_WITHOUT_GROUP_XML_TEMPLATE = (
    '      <module fileurl="file://{0}" filepath="{0}" />'
)
...
if __name__ == "__main__":
    ...
    with open(opts.prj or os.devnull, mode='w') as pf:
        ...
        for fn in walk(src, keep=is_iml, recur=negate(is_ignorable)):
            ...
            if iml_group == '':
                print(MODULE_WITHOUT_GROUP_XML_TEMPLATE.format(iml_path), file=pf)
            else:
                print(MODULE_WITH_GROUP_XML_TEMPLATE.format(iml_path, iml_group), file=pf)
